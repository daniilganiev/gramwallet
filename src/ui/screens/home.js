...
export function homeScreen(ctx) {
  ...
  const paintAssets = (jettons, nfts, fromCache = false) => {
    ...
    /*
     * Оба списка пришли и оба пусты. Вместо пустого места — строка о том,
     * что кошелёк умеет больше, чем сейчас показывает: иначе по экрану
     * кажется, что он только для GRAM.
     *
     * Проверяем именно длину массивов: null значит «не получили», а это
     * совсем другое дело, и обещать по нему нечего.
     */
    if (!blocks.length && jettons?.length === 0 && nfts?.length === 0) {
      blocks.push(el("p.assets__hint", { text: "Можно получать и отправлять токены и NFT" }));
    }
