...
export function settingsScreen(ctx) {
  ...
  return el("div.screen.stack", {}, [
    ...
    el("div.note.note--ok.settings__note", {}, [
      ...
      // Кошелёк живёт в памяти браузера, а не в аккаунте: на новом телефоне
      // его не окажется, и узнать об этом лучше заранее, а не в тот момент.
      el("div.note__more", {
        text: "К аккаунту Telegram кошелёк не привязан. На другом устройстве он не появится сам — войти можно только по seed-фразе, а после смены ключа ещё и по адресу.",
      }),
    ]),
