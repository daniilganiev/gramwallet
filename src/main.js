...
/**
 * Поле под клавиатурой — страховка на крайний случай.
 *
 * Экраны с вводом свёрстаны так, чтобы поле и так стояло выше линии
 * клавиатуры, и в норме здесь не происходит ничего. Но клавиатура на разных
 * телефонах разной высоты, и если поле всё же закрыто, поднимаем форму ровно
 * на недостающее.
 *
 * Поднимаем содержимое экрана, а не весь слой: нижние кнопки при этом стоят
 * на месте — их держит встречный сдвиг в стилях. Начав набирать код, человек
 * не собирается их нажимать, и таскать их за формой незачем.
 *
 * Прокрутка вместо подъёма не годится: она требует, чтобы содержимому было
 * куда ехать, а на экране блокировки его ровно в высоту экрана.
 *
...
function keepFocusVisible() {
  ...
  const place = (node, px) => {
    if (px) node.style.setProperty("--lift", `${px}px`);
  ...
  const adjust = () => {
    ...
    const next = field.closest(".modal__sheet, .screen");
    if (!next) return;
