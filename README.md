...
Реализованы возможности контракта, ради которых он и сделан.

Смена seed-фразы без смены адреса. Ключ меняется, адрес, баланс и история
остаются прежними — тем, кто вам платит, ничего сообщать не нужно. Повторять
можно сколько угодно раз.

До 255 переводов за одну транзакцию: сообщения кодируются змейкой чанков,
как того требует контракт.

Обновления без переезда. Логика живёт в сети, поэтому менять кошелёк, как
было с v3 и v4, не придётся.

Дальше — обычная работа кошелька.
